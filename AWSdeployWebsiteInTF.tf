...
#create a block volume fordata persistence
...
resource "aws_s3_bucket" "newbuck3t" {
  bucket = "newbuck3t"
 ...
 provisioner "local-exec" {
    command = "git clone https://github.com/Judewakim/DeployWebsiteOnAWSUsingTerraform.git web-server-image"
 }
...
resource "aws_s3_bucket_public_access_block" "public_storage" {
 depends_on = [aws_s3_bucket.newbuck3t]
 bucket = "newbuck3t"
...
resource "aws_cloudfront_distribution" "tera-cloufront1" {
    ...
    origin {
        domain_name = aws_s3_bucket.newbuck3t.bucket_regional_domain_name
